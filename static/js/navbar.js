function corverde() {
  $('.navbartotal').css('transition','all 0.5s ease-out');
  ...
  $('.navbartotal').css('box-shadow','0px 3px 7px rgba(0,0,0,0.3)');
}

function cortransparente() {
  $('.navbartotal').css('transition','all 0.5s ease-out');
  $('.navbartotal').css('box-shadow','none');
...
$(document).on('click', 'a[href^="#"]', function (event) {
  ...
  $('html, body').animate({
    scrollTop: $($.attr(this, 'href')).offset().top - ($(window).height() * 0.09)
  }, 800);
});
